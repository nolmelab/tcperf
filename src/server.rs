...
async fn run_tcp(args: &super::Args, tx: Sender<Event>) -> Result<(), anyhow::Error> {
    ...
    let event = Event::State("listening".into());
    let _ = tx.send(event);
        ...
        match result {
            Ok((stream, remote_addr)) => {
                ...
                tokio::spawn(async move {
                    let _ = run_stream(echo_size, stream, ntx).await;
                });
                count += 1;
            }
            Err(e) => {
                let event = Event::Error(format!("accept error:{:?}", e));
                let _ = tx.send(event);
            }
        }
    }
...
fn run_ui(args: &super::Args, rx: Receiver<Event>) -> Result<(), anyhow::Error> {
    ...
    while running {
        let result = terminal.draw(|f| f.render_widget(Paragraph::new(message.as_str()), f.size()));
        if let Err(e) = result {
        ...
        if let Ok(ev) = ev {
            ...
            match ev {
                ...
                Event::EchoRecv => {
                    echo_count += 1;
                    message = format!("Echo. Count: {}", echo_count);
                }
                Event::State(m) => {
                    message = m;
                }
